...
#
# Модуль используется и из bot3 (только релей новых постов через
# /updates — без автоматического бэкафилла истории при старте, чтобы
# редеплой не перезаливал старые посты повторно), и из bot2 (команда
# /copy_channel — ручной запуск бэкафилла истории по требованию, без
