...
# Модель DeepSeek. Прежняя "deepseek-chat" выключена на стороне API (отдаёт
# 400 invalid_request_error), из-за чего модерация молча пропускала вообще
# всё. Если точности flash не хватит — переключить на "deepseek-v4-pro"
# (через переменную окружения DEEPSEEK_MODEL, без правки кода).
...
ОСОБО СТРОГО — всё, что касается денег и финансов. Любое сообщение про
помощь с финансами, деньгами, кредитами, займами, долгами, банкротством,
кредитной историей, картами, инвестициями, криптовалютой, заработком или
доходом — это нарушение (BAN). Не важно, как оно подано: предложение,
намёк, вопрос, "просто рассказать", "консультация бесплатно" или пара
слов без подробностей. Здесь магазин товаров, а не финансовые услуги —
таким сообщениям в чате не место.

Примеры (все — BAN): "помогу с финансами", "решаю вопросы с долгами",
"списание кредитов", "помощь в получении займа", "есть вопросы по
финансам?", "научу зарабатывать", "инвестиции с гарантией".

Исключение: обычные бытовые упоминания денег в разговоре о товарах
магазина — "дорого", "а дешевле есть?", "картой можно оплатить?",
"сколько стоит" — это OK.
